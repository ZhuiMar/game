...
- (void)configView {
    ...
    self.thumbView.frame = CGRectMake(-7, 0, 28, 28);
...
- (void)layoutProgress {
    if(_setting.layoutDirection == YTSliderLayoutDirectionHorizontal) {
        ...
        self.progressView.frame = CGRectMake(progressStart, self.progressView.frame.origin.y-10,progressEnd - progressStart,trackHeight);
    }
    else {
        ...
        self.progressView.frame = CGRectMake(self.progressView.frame.origin.x, progressStart ,trackWidth, progressEnd - progressStart - 7);
        CGFloat curent = progressEnd - progressStart - 7;
        CGFloat value = (curent - 14.0)/(145 - 14);

        if([self.delegate respondsToSelector:@selector(ytSliderViewSlidering:value:)]){
            [self.delegate ytSliderViewSlidering:self value:value];
        }
...
- (UIView *)backgroundView {
    if(!_backgroundView) {
        ...
        image.layer.cornerRadius = thumbSizeValue/2;
        image.image = [UIImage imageNamed:@"圆角矩形"];
