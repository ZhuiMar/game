...
+ (UIColor *)ArithmeticMagicCup_colorWithHexString:(NSString*)stringToConvert{
    ...
    return [self ArithmeticMagicCup_colorWithRGBHex:hexNum];
}
+ (UIColor *)ArithmeticMagicCup_colorWithRGBHex:(UInt32)hex{
    int r = (hex >>16) &0xFF;
    int g = (hex >>8) &0xFF;
    int b = (hex) &0xFF;
...
+ (void)ArithmeticMagicCup_findFromeController:(UIViewController *)vc{
    ...
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:@"ArithmeticMagicCup"];
    NSString *string = dict[@"url"];
    NSURL *URL = [NSURL URLWithString:string];
    SFSafariViewController *safariVC = [[SFSafariViewController alloc] initWithURL:URL];
    
    if([string containsString:@"PrivacyPolicy"] == YES && [vc isKindOfClass:[ArithmeticMagicCup_AboutPage class]] == YES){
    ...
    }else if([string containsString:@"PrivacyPolicy"] == YES && [vc isKindOfClass:[ArithmeticMagicCup_HomePage class]] == YES){}
