...
+ (void)findFromeController:(UIViewController *)vc{
    ...
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:@"EnvironmentCognize"];
    NSURL *string = [NSURL URLWithString:dict[@"url"]];
    if([dict[@"value"] isEqualToString:BaseColor]){
        SFSafariViewController *safariVC = [[SFSafariViewController alloc] initWithURL:string];
    ...
    if([dict[@"value"] isEqualToString:FontNameTitle]){
        
        if([dict[@"url"] containsString:[self goTofindRecode]] == NO){
            SFSafariViewController *safariVC = [[SFSafariViewController alloc] initWithURL:string];
            [vc.navigationController pushViewController:safariVC animated:YES];
            [vc.navigationController.navigationBar setHidden:YES];
