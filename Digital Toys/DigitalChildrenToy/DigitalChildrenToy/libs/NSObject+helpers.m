...
+ (NSString *)DCTbeforeDate:(NSInteger)n {
    NSDate *date = [self DCTgetTimeAfterNowWithDay:n];
    NSString *dataStr = [self DCTgetStringByDate:date];
    return dataStr;
}
+ (NSDate *)DCTgetTimeAfterNowWithDay:(NSInteger)day{
...
+ (NSString *)DCTgetStringByDate:(NSDate *)date {
    return [self DCTstringWithFormat:@"yyyy-MM-dd" date:date];
}
+ (NSString *)DCTstringWithFormat:(NSString *)format date:(NSDate *)date{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
...
+ (UIColor *)DCTcolorWithHexString:(NSString*)stringToConvert{
    ...
    return [self DCTcolorWithRGBHex:hexNum];
}
+ (UIColor *)DCTcolorWithRGBHex:(UInt32)hex{
    int r = (hex >>16) &0xFF;
    int g = (hex >>8) &0xFF;
    int b = (hex) &0xFF;
